...
export function UserPositions() {
	...
	const totalVaultValue = mockVaultPositions.reduce((sum, pos) => {
		const value = parseFloat(pos.estimatedValue.replace(/[$,]/g, ''));
		return sum + value;
	}, 0);
	...
	const hasPositions =
		mockVaultPositions.length > 0 || tokenPositions.length > 0;
						...
						<div className='flex items-center gap-3'>
							...
							<div>
								...
								<p className='text-sm text-slate-500 dark:text-slate-400'>
									{mockVaultPositions.length + tokenPositions.length} active
									position
									{mockVaultPositions.length + tokenPositions.length !== 1
										? 's'
										: ''}
								</p>
							</div>
			...
			<Tabs defaultValue='vaults' className='w-full'>
				<TabsList className='grid w-full grid-cols-2'>
					<TabsTrigger value='vaults' className='flex items-center gap-2'>
						🏦 Vault Positions ({mockVaultPositions.length})
					</TabsTrigger>
