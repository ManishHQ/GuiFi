...
/**
 * Get prediction market factory address for a specific chain
 */
export function getPredictionMarketFactoryAddress(
	chainName: ChainName
): string | null {
	const config = getChainConfig(chainName);
	return config?.predictionMarkets?.PredictionMarketFactory || null;
}

/**
 * Get prediction market factory address by chain ID
 */
export function getPredictionMarketFactoryAddressById(
	chainId: number
): string | null {
	const chainInfo = getChainConfigById(chainId);
